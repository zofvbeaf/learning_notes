...
+ `linux`内核实现的自旋锁是不可递归的，也就是说当你获得了这个锁想再自选 的获取它就会陷入死锁，即双重请求死锁
+ 自旋锁可以用在中断处理程序中（此处不能使用信号量，因为它们会导致睡眠），中断处理程序请求使用自旋锁时一定要先禁止中断（在当前处理器上的中断请求），否则可能导致被再次中断，导致双重请求死锁，当然在不同的处理器上是可以双重请求的

### 读写自旋锁

+ 写入时不能读写，读取时可以读但不能写
+ 代码中先获取读锁再获取死锁会造成死锁，也就是不能把一个读锁升级为写锁
+ 大量的读者会使写者处于饥饿状态

### 信号量

+ 等待锁可用时会睡眠，适用于锁会被长时间持有的情况，锁持有时间短则划不来，因为睡眠、维护等待队列以及唤醒所花费的开销可能比锁占用的全部时间还要长
+ 由于会睡眠，所以只能再进程上下文中获取信号量锁，因为在中断上下文中是不能进行调度的
+ 占用信号量的同时不能占用自旋锁，因为可能会睡眠
+ 信号量分为计数信号量（可以有多个持有者）和二值信号量（只允许一个持有者）
+ 信号量支持两个原子操作：`P()`和`V()`，`linux`中对应为`down()`和`up()`，分别表示计数减一（获得信号量）和加一（释放信号量）
+ 如果在信号量上的等待队列不为空，那么处于队列中等待的任务在被唤醒的同时会获得信号量
+ 另外内核也有相应的**读写信号量**，并且有`downgrade_wirte()`接口可以动态地将写锁转为读锁

### mutex

+ 相比二值信号量，其接口更简单，实现更高效，而且使用限制更强
+ 不能递归的上锁和解锁，当持有一个`mutex`时，进程不可以退出
+ `mutex`不能在中断或者下半部中使用，即使使用`mutex_trylock()也`不行
  + 一般用`mutex`更多，除非其不能满足约束条件，再考虑信号量

### 完成变量

+ 类似条件变量
  + `init_completion(struct completion *)`：初始化制定的动态创建的完成变量
...
+ 写数据时更新数据的`seq`号，读数据时记录当前读的`seq`号版本，再次读时若版本不一样则重新读，适合写多的场景，相当于把重负载都丢给了读操作处理，而RCU相当于把重负载的操作都丢给写操作处理。
...
+ 系统定时器的频率是通过静态预处理定义的，也就是`HZ`，在系统启动时按照HZ值对硬件进行设置，`x86`为`100`，即每`10ms`产生一次时钟中断，在`4.4`内核的`ubuntu`阿里云服务器上配置为`CONFIG_HZ=250`
+ 内核中有一个全局变量`jiffies`存储了自系统启动以来产生的街拍的总数
+ 实时时钟（`RTC`）是硬件时钟，由主板电池供电计时。系统启动时，内核读取`RTC`到`xtime`中来初始化墙上时间（即实际时间），`x86`也会周期性的将当前时间值存回`RTC`中
+ `x86`体系结构主要采用可编程中断时钟（`PIT`）来实现系统定时器，即以`HZ/秒`的频率产生时钟中断
+ `gettimeofday`的实现即是通过`syscall`读取时间
+ 内核在时钟中断发生后执行定时器，定时器作为软中断在下半部上下文中执行，所有的注册的定时器以链表的形式存放在一起，`2.6`以时间轮的形式组织
+ 内核定时器不能用来实现硬实时任务，因为时钟中断的间隔一般为几`ms`的固定值
